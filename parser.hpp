...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YY_ASSERT (tok == token::DECIMAL || tok == token::OCTAL || tok == token::HEXADECIMAL);
      }
#else
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      make_OCTAL (int v)
      {
        return symbol_type (token::OCTAL, std::move (v));
      }
#else
      static
      symbol_type
      make_OCTAL (const int& v)
      {
        return symbol_type (token::OCTAL, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_HEXADECIMAL (int v)
      {
        return symbol_type (token::HEXADECIMAL, std::move (v));
      }
#else
      static
      symbol_type
      make_HEXADECIMAL (const int& v)
      {
